...
(function ($) {
    $.extend({
    	...
    	table: {
			...
			// 图片预览 原图预览
			imageViewOri: function (value, height, width, target) {
				if ($.common.isEmpty(width)) {
					width = 'auto';
				}
				if ($.common.isEmpty(height)) {
				...
				var _target = $.common.isEmpty(target) ? 'self' : target;
				if ($.common.isNotEmpty(value)) {
					return $.common.sprintf("<img height='198px' class = 'center-block' data-width='100%' data-target='%s' src='%s'/>", _target, value);
				} else {
					return $.common.nullToStr(value);
				}
			},
    	...
    	modal: {
			...
			// 选卡页方式打开
			openIndexTab: function (title, url,panel) {
				createMenuItemOnIndex(url, title,panel);
			},
            // 选卡页同一页签打开
